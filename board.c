...
int CheckBoard(const S_BOARD *pos) {
	...
	ASSERT(pos->enPas==NO_SQ || (RanksBrd[pos->enPas]==R_6 && pos->side ==WHITE) || ( RanksBrd[pos->enPas]==R_3 && pos->side ==BLACK));
...
void UpdateListsMaterial(S_BOARD *pos) {
	...
	for(index = 0; index < BRD_SQ_NUM; ++index) {
		...
		if(piece!=OFFBOARD && piece!= EMPTY) {
			...
			//piece list
			//pList[piece type][pceNum] = square
...
int Parse_FEN(char *fen, S_BOARD *pos) {
	...
	if(*fen != '-') {
		...
		ASSERT(file>=F_A && file<=F_H);
		ASSERT(rank>=R_1 && rank <= R_8);
